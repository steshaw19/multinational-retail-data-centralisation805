...
class DataCleaning:
    def clean_user_data(self, user_data_df):
        try:
            ...
            # Drop any names that contain numbers
            user_data_df = user_data_df[user_data_df['first_name', 'last_name'].str.isalpha()]
            ...
            no_number_mask_company = ~user_data_df['column'].str.contains(r'\d', na=False)
            # Filter the DataFrame using the boolean mask
            ...
            valid_country_code_mask = user_data_df['country'].str.len().isin([2, 3])
            # Filter the DataFrame using the boolean mask
